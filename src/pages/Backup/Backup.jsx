...
const Backup = () => {
  ...
  const initLocalDirectory = async (directoryHandle, prompt = true) => {
    ...
    if (permissions) {
      ...
      alert(
        "Failed to set up local backup. Reach out to us at support@demokraft.ai for more help. You can still record your screen."
      );
  ...
  const localSaving = async (prompt = true) => {
    ...
    if ("showDirectoryPicker" in window) {
      ...
      alert(
        "Your browser doesn't support local backups. Reach out to us atsupport@demokraft.ai for more help. You can still record your screen."
      );
      chrome.storage.local.set({ backup: false });
      chrome.runtime.sendMessage({
        type: "backup-created",
